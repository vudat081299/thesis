...
//        }
//    }
    var avatar: UIImage? = AuthenticationService.avatar ?? nil
    var authUser: User? = AuthenticationService.user ?? nil
    ...
    @objc func leftBarItemAction() {
        print("Right bar button was pressed!")
        AuthenticationService.logout(on: self)
    }
        ...
        if avatar != nil {
            ...
            updateAvatarRequest.put(token: true, fileUploadData) { result in
                ...
                case .success:
                    ...
                    AuthenticationService.prepareUserProfileData()
...
//        self.refreshData()
        AuthenticationService.prepareUserProfileData() { [weak self] avatar, userProfileData in
            if let self = self {
                DispatchQueue.main.async {
    ...
    func refreshData() {
        avatar = AuthenticationService.avatar
        authUser = AuthenticationService.user
