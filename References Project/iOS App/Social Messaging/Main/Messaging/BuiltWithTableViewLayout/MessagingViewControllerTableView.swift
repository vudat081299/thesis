...
class MessagingViewControllerTableView: UIViewController, MessagePullThread, MessagePushThread {
    ...
    var userBoxData: [ResolvedBox] = AuthenticationService.userBoxData
    let authUser: User? = AuthenticationService.user
...
//        }
    ...
    */
    ...
    func fetchBoxesData(completion: @escaping () -> Void) {
        guard let userObjectId = authUser?.id
        else {
            return
        }
        ...
        request_box.getArray(token: true) { result in
            ...
            case .success(let data):
                ...
                AuthenticationService.userBoxData = boxData
                self.userBoxData = boxData
                completion()
            case .failure:
                break
    ...
    func receiveMessage(message: WebSocketMessage) {
        messagePullThreadDelegate?.receiveMessage(message: data)
    }
...
//        cell.boxImage
        ...
        if let authUser = AuthenticationService.user, box.type == .privateChat {
            ...
            for (index, userObjectId) in box.members_id.enumerated() {
                if userObjectId != authUser.id.uuidString {
                    if let image = cacheImages[userObjectId] {
                        cell.boxImage.image = image
                    } else {
                        let imageURL = "\(baseURL)users/getavatarwithuserobjectid/\(userObjectId)" // Constant.
...
//        }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if box.type == .privateChat {
            chattingViewController.title = box.membersName.first(where: { $0 != AuthenticationService.user?.name })
        } else {
