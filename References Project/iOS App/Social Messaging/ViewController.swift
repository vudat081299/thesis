...
//                return collectionView.dequeueConfiguredReusableCell(using: customCellRegistration, for: indexPath, item: identifier)
            case .collection:
                ...
                if user.id == AuthenticationService.user!.id {
                    self.i = 1
                    user = self.resolvedUser[indexPath.row + self.i]
//                    let imageURL = "\(baseURL)users/getavatarwithuserobjectid/\(user._id!)"
//                    if let image = imageURL.getImageWithThisURL() {
//                        DispatchQueue.main.async {
////                            cacheImages[user.id!] = image
//                        }
//                    }
                }
...
//                    let imageURL = "\(baseURL)users/getavatarwithuserobjectid/\(user._id!)" // Constant.
...
//                    }
        ...
        func createBox(withDataOf cell: UserView) {
            guard let user = cell.userProfileData else { return }
            RequestService.createChatBoxes(with: user.id)
        }
