...
class TabBarController: UITabBarController, MessagePushThread {
    ...
    static let ws = WebSocketSM("ws://\(domain!)/connectws/\(AuthenticationService.user?.id.uuidString ?? "")")
    
    // MARK: - Delegate methods.
    func sendMessage(data: MessageSendWS) {
        let encoder = JSONEncoder()
        guard let data = try? encoder.encode(data) else { return }
        let ws = WebSocketSM("ws://\(domain!)/connectws/\(AuthenticationService.user?.id.uuidString ?? "")")
        ws.send(data)
    }
...
//                decoder.dateDecodingStrategy = .secondsSince1970
                ...
                print("Resolved WS Data successful!")
                    self.messagePullThreadDelegate?.receiveMessage(message: resolvedData)
